...
const TreatmentSelectionRequestSchema = z.strictObject({
  expectedProjectRevision: z.number().int().positive(),
  confirmed: z.literal(true),
});

interface RouteContext {
  params: Promise<{ projectId: string; treatmentId: string }>;
}

export async function POST(request: Request, context: RouteContext) {
  const paramsResult = z
    .strictObject({ projectId: IdSchema, treatmentId: IdSchema })
    .safeParse(await context.params);
  if (!paramsResult.success) {
    return errorResponse(
      "workflow_path_invalid",
      "The project or Director Treatment identifier is invalid.",
      400,
    );
  }
  ...
  try {
    ...
    return errorResponse(
      ...
      "The Treatment selection request must be valid JSON.",
      400,
    );
  }
  const bodyResult = TreatmentSelectionRequestSchema.safeParse(requestBody);
  if (!bodyResult.success) {
    return errorResponse(
      "workflow_request_invalid",
      "Explicitly confirm the new direction and include the current project revision.",
      400,
    );
  }
  ...
  try {
    ...
    console.error("Treatment selection persistence failed.", error);
    return errorResponse(
